...
class _WindowsAdbServerStartError(RuntimeError):
    """Infrastructure failure while establishing the Windows ADB server."""


class _WindowsAdbServerRetainedStartError(_WindowsAdbServerStartError):
...
def _resolve_executable(
    ...
    if resolved is None:
        raise _WindowsAdbServerStartError(
...
_WindowsAdbServerResource: TypeAlias = (
    socket.socket | WindowsOwnedProcess | WindowsRetainedProcess
)


class _WindowsAdbServerFactory:
    ...
    def acquire(
    ...
    ) -> RequirementAcquireResult[_WindowsAdbServerResource]:
        ...
        resources: list[_WindowsAdbServerResource] = []
        try:
            ...
            try:
                ...
                raise _WindowsAdbServerStartError(str(exc)) from exc
        ...
        except _WindowsAdbServerRetainedStartError as exc:
        ...
        except _WindowsAdbServerStartError as exc:
            return RequirementAcquireFailed(
                AdbServerAcquireError(str(exc)),
                tuple(resources),
            )
    ...
    def cleanup(
        self,
        resources: PhysicalResources[_WindowsAdbServerResource],
    ) -> None:
    ...
    def _reserve_listener(self, server_endpoint: TcpEndpoint) -> socket.socket:
        ...
        try:
            ...
            try:
                ...
                raise _WindowsAdbServerRetainedStartError(
            ...
            if isinstance(exc, _WindowsAdbServerStartError):
                raise
            if not isinstance(exc, Exception):
                raise
            raise _WindowsAdbServerStartError(
    ...
    def _check_startup(self, deadline: float) -> None:
        if self._monotonic() >= deadline:
            raise _WindowsAdbServerStartError("ADB server startup timed out")
