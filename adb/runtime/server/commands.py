...
from lifecycle.capability.supervision.acquire import AcquireDisposition, classify_acquire_result
...
from lifecycle.capability.supervision.release import ReleaseDisposition, classify_release_result
...
from adb.server.supervision import (
...
class AdbServerDeactivateIncomplete:
    """Report bounded or host-required deactivation with ownership retained."""
...
AdbServerDeactivateResult: TypeAlias = AdbServerLifecycleResult | AdbServerDeactivateIncomplete


class AdbServerCommands:
    """Serialize caller-facing activation/deactivation over one lifecycle owner."""
    ...
    def __init__(
        ...
        *,
        ...
        self._policy = policy
        self._clock = _clock
        self._acquire = AdbServerAcquireSupervisor(lifecycle, _sleeper=_sleeper, _clock=_clock)
        self._recovery = AdbServerRecoverySupervisor(lifecycle, _sleeper=_sleeper, _clock=_clock)
        self._release = AdbServerReleaseSupervisor(lifecycle, _sleeper=_sleeper, _clock=_clock)
        self._lock = RLock()
        ...
        *,
        ...
        while True:
            ...
            if deadline is not None:
                wait_seconds = min(wait_seconds, max(0.0, deadline - self._clock()))
            if wait_seconds <= 0.0:
    ...
    def activate(
        ...
        *,
        ...
        """Activate without implicitly recovering pending cleanup or replacing Active."""
        ...
        try:
            ...
            if snapshot.phase in (
                ...
                return LifecycleResult.not_executed(snapshot)

            remaining = self._remaining_timeout(deadline)
            if remaining is not None and remaining <= 0.0:
                return AdbServerActivateIncomplete(
                    self._lifecycle.read(), SupervisionStopReason.TIMED_OUT
                )
            ...
            disposition = classify_acquire_result(result, snapshot.generation, request)
            if disposition is AcquireDisposition.GENERATION_MISMATCH:
                raise RuntimeError("server lifecycle generation changed outside command surface")
            return result
        finally:
            self._lock.release()
    ...
    def deactivate(
        ...
        *,
        ...
        """Release Active or continue pending cleanup/finalization under one deadline."""
        ...
        try:
            ...
            if snapshot.phase in (
                AdbServerPhase.CLEANUP_PENDING,
                AdbServerPhase.FINALIZATION_PENDING,
                AdbServerPhase.RECOVERING,
            ):
                result = self._recovery.supervise(
                    snapshot.generation,
                    snapshot.request,
                    timeout_seconds=remaining,
                    cancellation=cancellation,
                ...
                disposition = classify_recovery_result(
                    result, snapshot.generation, snapshot.request
                )
                if disposition is RecoveryDisposition.GENERATION_MISMATCH:
                    raise RuntimeError("server lifecycle generation changed outside command surface")
                if disposition is RecoveryDisposition.REQUEST_MISMATCH:
                    raise RuntimeError("server lifecycle request changed outside command surface")
                return result
